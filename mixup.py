...
def mixup_cross_entropy_loss(input, target, size_average=True):
    ...
    input = torch.log(torch.nn.functional.softmax(input, dim=1).clamp(0.001, 1))
...
def mixup2(inputs, feats, targets, num_classes, alpha=2):
    """Mixup on 1x32x32 mel-spectrograms.
    """
    s = inputs.size()[0]
    weight = torch.Tensor(np.random.beta(alpha, alpha, s))
    ...
    # feats
    weight = weight.view(s, 1)
    x1, x2 = feats, feats[index, :]
    feats = weight*x1 + (1-weight)*x2
    weight = weight.view(s, 1)
    targets = weight*y1 + (1-weight)*y2
    return inputs, feats, targets